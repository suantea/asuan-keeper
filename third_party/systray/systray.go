...
var (
	...
	// 托盘图标鼠标事件回调（asuan 扩展）：
	// Windows 上区分左键单击（查看进度）/左键双击（打开配置）/右键（菜单）。
	leftClickFn  func()
	dClickFn     func()
	leftClickMu  sync.RWMutex
	dClickMu     sync.RWMutex
)
...
// SetLeftClickFn 设置托盘图标左键单击回调（asuan 扩展）。
// Windows 上左键单击查看同步进度；macOS 上单击弹菜单，此回调在
// 双击(或非菜单平台)等场景兜底，可留空。
func SetLeftClickFn(fn func()) {
	leftClickMu.Lock()
	leftClickFn = fn
	leftClickMu.Unlock()
}

// SetDClickFn 设置托盘图标左键双击回调（asuan 扩展）。
// Windows 上左键双击打开配置界面。
func SetDClickFn(fn func()) {
	dClickMu.Lock()
	dClickFn = fn
	dClickMu.Unlock()
}

// systrayLeftClick 由平台事件循环调用：触发左键单击回调。
...
// systrayDClick 由平台事件循环调用：触发左键双击回调。
func systrayDClick() {
	dClickMu.RLock()
	fn := dClickFn
	dClickMu.RUnlock()
