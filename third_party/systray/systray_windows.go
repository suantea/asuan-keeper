...
type winTray struct {
	...
	// lastLeftClickMs 上次左键单击时间戳（毫秒），用于过滤双击后
	// 附带触发的 WM_LBUTTONUP（双击 300ms 内不再触发单击回调）。
	lastLeftClickMs int64
...
func (t *winTray) wndProc(hWnd windows.Handle, message uint32, wParam, lParam uintptr) (lResult uintptr) {
	...
	case t.wmSystrayMessage:
		...
		case WM_LBUTTONUP:
			// 左键单击：查看同步进度。
			// 双击时会先触发一次 WM_LBUTTONUP 再触发 WM_LBUTTONDBLCLK，
			// 用时间窗口过滤：300ms 内收到双击则忽略该单击。
			now := time.Now().UnixMilli()
			if now-t.lastLeftClickMs > 300 {
				systrayLeftClick()
			}
			t.lastLeftClickMs = now
		}
